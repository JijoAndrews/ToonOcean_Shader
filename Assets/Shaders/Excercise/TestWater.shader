		...
		SubShader
		{
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					half depth = tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(i.scrPos)).r;
					depth = LinearEyeDepth(depth);
					half depthDiff = depth - i.scrPos.w;

					//half depth = LinearEyeDepth(tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD( i.scrPos)).r);
					//float objectZ = i.scrPos.z;

					half4 foamLine = 1 - saturate(_Foam *depthDiff);// foam line by comparing depth and screenposition
					//half4 foamLine = 1 - saturate(_Foam * (depth - i.scrPos.w));// foam line by comparing depth and screenposition//og
